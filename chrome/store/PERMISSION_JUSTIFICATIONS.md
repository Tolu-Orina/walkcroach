...
| Host permission for WalkCroach API only (`https://<api-host>/*`, e.g. `https://awbcf4clij.execute-api.eu-west-2.amazonaws.com/*`) | Call our HTTPS Chrome backend (device session, summarize, ask, draft, save, recall). **Not** used to read arbitrary websites. |
